...
char mqtt_broker_ip[64] = MQTT_BROKER_IP; // Default MQTT broker IP address
...
void mqtt_do_reconnect(mqtt_client_t *client)
{
...
void mqtt_do_connect(mqtt_client_t *client, char *broker_ip)
{
    ...
    ci.client_id = "feed_control_client";
...
void mqtt_connection_cb(mqtt_client_t *client, void *arg, mqtt_connection_status_t status)
{
    ...
    if (status == MQTT_CONNECT_ACCEPTED)
    {
        ...
        err = mqtt_subscribe(client, FEED_DISPENSER_TOPIC, 1, mqtt_sub_request_cb, arg);
        if (err != ERR_OK)
        {
            printf("mqtt_subscribe return: %d\n", err);
        }

        err = mqtt_subscribe(client, CONTROL_MQTT_TOPIC, 1, mqtt_sub_request_cb, arg);
        if (err != ERR_OK)
        {
            printf("mqtt_subscribe return: %d\n", err);
        }
...
static void mqtt_incoming_publish_cb(void *arg, const char *topic, u32_t tot_len)
{
    ...
    else if (strcmp(topic, FEED_DISPENSER_TOPIC) == 0)
    {
        inpub_id = 1;
    }
    else if (strcmp(topic, CONTROL_MQTT_TOPIC) == 0)
    {
        inpub_id = 2;
    }
    else
...
static void mqtt_incoming_data_cb(void *arg, const u8_t *data, u16_t len, u8_t flags)
{
    ...
    if (flags & MQTT_DATA_FLAG_LAST)
    {
        ...
        {
            feet_dispenser_topic_cb(message, len);
            // The message contains the feed dispenser nummber feed turns
            int feed_turns = atoi(message);

            if (feed_turns > 0)
            {
                printf("mqtt_incoming_data_cb: Feed dispenser command received, dispensing %d turns...\n", feed_turns);
            }
        }
        else if (inpub_id == 2)
        {
            // The commands here are for control mqtt parameters
            // COMMANDS: "SET_MQTT_BROKER"
            if (strncmp(message, "SET_MQTT_BROKER=", 16) == 0)
            {
                // Extract the new MQTT broker address from the message
                char new_broker[64];
                sscanf(message + 16, "%63s", new_broker);
                printf("mqtt_incoming_data_cb: Setting new MQTT broker address: %s\n", new_broker);
            }
            else
            {
                printf("mqtt_incoming_data_cb: MQTT Unknown control command received: %s\n", message);
            }
        }
        else
        {
...
void mqtt_health_check(mqtt_client_t *client)
{
    ...
    if (current_time - last_health_check > 30 * 1000)
    {
        bool connected = mqtt_client_is_connected(client);
        if (connected)
        {
            printf("mqtt_health_check: Client is connected, publishing health status...\n");
