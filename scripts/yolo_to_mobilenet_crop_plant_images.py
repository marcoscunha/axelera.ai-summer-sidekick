...
BASE_PATH = "datasets/plant_health/Healthy-Unhealthy-Plant.v2i.yolov11"
DATA_PATH = f"{BASE_PATH}/data.yaml"
...
output_paths = [
    f"{BASE_PATH}/train/cropped/",
    f"{BASE_PATH}/valid/cropped/",
    f"{BASE_PATH}/test/cropped/"
]


# Load class names from notes.json
with open(DATA_PATH, "r") as f:
    # Read YAML file and extract class names
    data = yaml.safe_load(f)
    class_names = data.get("names", [])
    class_map = {i: name for i, name in enumerate(class_names)}


for images_path, labels_path, output_path in zip(images_paths, labels_paths, output_paths):
